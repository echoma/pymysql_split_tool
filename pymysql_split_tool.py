...
'''
output progress during task processing
'''
last_step = 0
step_size = 10000
def output_step(row_count, final=False):
    global last_step
    step = int(row_count/step_size)
    if final or (row_count!=0 and step!=last_step):
...
if input.action=='split':
    group_method = input.task['rule']['group_method']
    ...
    if 'mysql' in input.task['dest']:
        ...
        input.cal_group_func(group_method)
            ...
            while True:
                ...
                while True:
                    ...
                    for one in many:
                        ...
                        db.replace_into_new_table(insert_cursor, input.cal_new_table_name(n), one)
                    db.db_dest.commit()
            ...
            for group_int_n in input.group_int_list:
                ...
                while True:
                    ...
                    if 'filter' in input.task['rule']:
                        ...
                        sql += " where "
                    sql += input.cal_group_filter_sql(group_method, group_int_n)
                    if 'order_by' in input.task['rule']:
                        sql += " order by "+input.task['rule']['order_by']
                    if 'page_size' in input.task['rule']:
                        sql += " limit "+str(offset)+","+str(input.task['rule']['page_size'])
                    ...
                    if cursor.rowcount==0:
                        break
                    new_table_name = input.cal_new_table_name(group_int_n)
                    while True:
                        many = cursor.fetchmany()
                        if len(many)==0:
                            break
        ...
        if 'group_int' not in input.task['rule']:
            ...
            sql = 'select '+input.cal_group_field_sql(group_method)+" from `"+src_database_name+"`.`"+src_table_name+"` "+input.cal_group_by_sql(group_method)
